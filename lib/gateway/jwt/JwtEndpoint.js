...
module.exports = (() => {
	...
	class JwtEndpoint {
		...
		/**
		 * Creates and starts a new {@link JwtEndpoint} for use in the test environment.
		 *
		 * @public
		 * @static
		 * @param {String} user - The identifier of the user to impersonate.
		 ...
		 */
		static forTest(user) {
			return EndpointBuilder.for('read-jwt-token-for-test', 'lookup user identity')
				.withVerb(VerbType.GET)
				.withProtocol(ProtocolType.HTTPS)
				.withHost(Configuration.testHost)
				.withPathBuilder((pb) =>
					pb.withLiteralParameter('version', 'v1').withLiteralParameter('token', 'token')
				)
				.withQueryBuilder((qb) => qb.withLiteralParameter('user', 'userId', user))
