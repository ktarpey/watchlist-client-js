...
module.exports = (() => {
	...
	class JwtGateway extends Disposable {
		...
		/**
		 * Creates and starts a new {@link RequestInterceptor} for use in the testing environment.
		 *
		 * @public
		 * @static
		 * @param {String} userId - The identifier of the user to impersonate.
		 * @returns {Promise<RequestInterceptor>}
		 */
		static forTestClient(userId) {
			return Promise.resolve()
				.then(() => {
					assert.argumentIsRequired(userId, 'userId', String);

					const gateway = new JwtGateway(JwtEndpoint.forTest(userId), 300000);
