...
module.exports = (() => {
	...
	class WatchlistGateway extends Disposable {
		constructor(protocol, host, port, environment, requestInterceptor, wsProtocol, wsHost, wsPort) {
			...
			this._editPreferencesEndpoint = EndpointBuilder.for('edit-preferences', 'edit preferences')
				.withVerb(VerbType.PUT)
				.withProtocol(protocolType)
				.withHost(host)
				.withPort(port)
		...
		editPreferences(watchlist, preferences) {
			return Promise.resolve()
				.then(() => {
					...
					return Gateway.invoke(this._editPreferencesEndpoint, { watchlist, preferences });
				});
		}
		...
		static forDevelopment(requestInterceptor) {
			return Promise.resolve(requestInterceptor)
				.then((requestInterceptor) => {
					...
					return start(new WatchlistGateway('https', Configuration.developmentHost, 443, 'development', requestInterceptor, 'wss', Configuration.developmentHostForWebSocket));
				});
		}
		...
		static forStaging(requestInterceptor) {
			return Promise.resolve(requestInterceptor)
				.then((requestInterceptor) => {
					...
					return start(new WatchlistGateway('https', Configuration.stagingHost, 443, 'staging', requestInterceptor, 'wss', Configuration.stagingHostForWebSocket));
				});
		}
		...
		static forProduction(requestInterceptor) {
			return Promise.resolve(requestInterceptor)
				.then((requestInterceptor) => {
					...
					return start(new WatchlistGateway('https', Configuration.productionHost, 443, 'production', requestInterceptor, 'wss', Configuration.productionHostForWebSocket));
				});
		}
