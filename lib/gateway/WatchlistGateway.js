...
module.exports = (() => {
	...
	class WatchlistGateway extends Disposable {
		constructor(protocol, host, port, environment, requestInterceptor) {
			...
			this._writeUserEndpoint = EndpointBuilder.for('write-user', 'save your watchlists')
				...
				.withPort(port)
				.withPathBuilder((pb) =>
					pb.withLiteralParameter('version', 'v1').withLiteralParameter('user', 'user')
				)
		...
		/**
		 * Retrieves the watchlist from the remote server.
		 *
		 * @public
		...
		/**
		 * Instructs the remote server to save the watchlist.
		 *
		 * @public
		...
		writeUser(watchlist) {
			return Promise.resolve().then(() => {
				assert.argumentIsRequired(watchlist, 'watchlist', Object, 'Object');

				checkStart.call(this);

				return Gateway.invoke(this._writeUserEndpoint, watchlist);
			});
		}
		...
		static forDevelopment(requestInterceptor) {
			return Promise.resolve(requestInterceptor).then((requestInterceptor) => {
				...
				return start(
					new WatchlistGateway('https', Configuration.developmentHost, 443, 'development', requestInterceptor)
				);
			});
		}
		...
		static forStaging(requestInterceptor) {
			return Promise.resolve(requestInterceptor).then((requestInterceptor) => {
				...
				return start(
					new WatchlistGateway('https', Configuration.stagingHost, 443, 'staging', requestInterceptor)
				);
			});
		}
		...
		static forProduction(requestInterceptor) {
			return Promise.resolve(requestInterceptor).then((requestInterceptor) => {
				...
				return start(
					new WatchlistGateway('https', Configuration.productionHost, 443, 'production', requestInterceptor)
				);
			});
		}
	...
	const requestInterceptorForSerialization = RequestInterceptor.fromDelegate((request, ignored) => {
		assert.argumentIsRequired(request.data, 'request.data', Object, 'Object');
