...
module.exports = (() => {
	...
	class WatchlistGateway extends Disposable {
		constructor(protocol, host, port, environment, requestInterceptor, webSocketProtocol, webSocketHost) {
			...
			this._webSocketInterceptor = function() {
		 ...
		 *
		 * @public
		 * @param {Callbacks.WebSocketDataReceiveCallback} dataReceivedCallback
		 * @param {Function} refreshCallback - a callback function to refresh application state
		 * @returns {*}
		 */
		subscribe(dataReceivedCallback, refreshCallback) {
			return Promise.resolve()
				.then(() => {
					if (!this._isSubscriberExist && window.WebSocket) {
						return this._webSocketInterceptor()
							.then((token) => {
