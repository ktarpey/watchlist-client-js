	...
	class WatchlistGateway extends Disposable {
		constructor(protocol, host, port, requestInterceptor) {
			...
			this._readServiceMetadataEndpoint = EndpointBuilder.for('read-service-metadata', 'check watchlist service status')
				...
				.withErrorInterceptor(ErrorInterceptor.LAMBDA_AUTHORIZATION_FAILURE)
				.endpoint;

			this._readUserEndpoint = EndpointBuilder.for('read-user', 'query user watchlist')
				.withVerb(VerbType.GET)
				.withProtocol(protocolType)
				.withHost(host)
				.withPort(port)
			...
			this._writeUserEndpoint = EndpointBuilder.for('write-user', 'save user watchlist')
